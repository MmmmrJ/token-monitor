...
- 点击齿轮设置语言、额度来源、始终置顶和登录时启动；登录时启动开关以系统真实启动项状态为准，失败时在设置项内显示错误。
- 关闭主窗口（含 Windows 关闭按钮、Alt+F4、macOS Cmd+W）会隐藏到系统托盘，不会结束进程；托盘“退出”或 macOS Cmd+Q 才会真正退出。
- 重复启动会复用已有进程并恢复主窗口，不会产生第二个实例。
- 关闭主窗口后可通过系统托盘重新打开、刷新或退出应用。

### 从旧版升级

若本机仍保留旧名称 `Codex Usage Monitor` 的登录启动项，v1.1.3 会在启动时先创建 `Token Monitor` 启动项并验证成功，再删除旧项。新项创建失败时会保留旧项，并在设置中显示内联错误。

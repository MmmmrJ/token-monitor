...
const copy = {
  zh: {
    localAccount: '本机账户', fiveRemaining: '5 小时剩余', weekRemaining: '7 天剩余', nextReset: '下次重置',
    dualAria: '双环额度视图', focusAria: '聚焦额度视图',
    ...
    nativeOnly: '请运行桌面应用以读取本机 Codex 登录态。', missingBoth: '已连接，但当前账户没有下发 5 小时或 7 天额度窗口。',
    missingFive: '已连接；当前账户暂未下发 5 小时额度窗口。', missingWeek: '已连接；当前账户暂未下发 7 天额度窗口。',
    authMissing: '未找到本机 Codex 登录态，请先运行 codex login。', reauth: 'Codex 登录态已过期，请重新运行 codex login。',
    unsupportedAuth: '当前为 API Key 模式；请使用 ChatGPT 登录以读取订阅额度。', networkError: '暂时无法连接额度服务，请检查网络。',
    serviceError: '额度服务暂时不可用，请稍后重试。', invalidResponse: '额度数据格式已变化，当前无法解析。', staleNotice: '刷新失败，正在显示上次成功数据。',
    preview: '设计预览数据', noReset: '无重置时间', unknownAccount: '本机 Codex 账户'
  },
  en: {
    localAccount: 'Local account', fiveRemaining: '5-hour remaining', weekRemaining: '7-day remaining', nextReset: 'Next reset',
    dualAria: 'Dual-ring quota view', focusAria: 'Focused quota view',
    ...
    nativeOnly: 'Run the desktop app to read the local Codex sign-in session.', missingBoth: 'Connected, but this account did not return a 5-hour or 7-day quota window.',
    missingFive: 'Connected; this account did not return a 5-hour quota window.', missingWeek: 'Connected; this account did not return a 7-day quota window.',
    authMissing: 'No local Codex sign-in was found. Run codex login first.', reauth: 'The Codex sign-in has expired. Run codex login again.',
    unsupportedAuth: 'Codex is using API-key mode. Sign in with ChatGPT to read subscription limits.', networkError: 'The quota service could not be reached. Check the network.',
    serviceError: 'The quota service is temporarily unavailable.', invalidResponse: 'The quota response format changed and could not be parsed.', staleNotice: 'Refresh failed. Showing the last successful data.',
    preview: 'Design preview data', noReset: 'No reset time', unknownAccount: 'Local Codex account'
  }
};
...
function previewSnapshot() {
  const now = Date.now();
  const quota = (remainingPercent, durationSeconds, resetMs) => ({
  ...
  return {
    ...
    provider: { connected: true, state: 'preview', message: t('preview'), source: 'preview', authPathLabel: '~/.codex/auth.json' },
...
function formatReset(windowData) {
  ...
  if (sameDay) return preferences.language === 'zh' ? `今天 ${time} ${t('resets')}` : `Today ${time} ${t('resets')}`;
  const day = new Intl.DateTimeFormat(preferences.language === 'zh' ? 'zh-CN' : 'en', { month: 'short', day: 'numeric' }).format(date);
  return `${day} ${time} ${t('resets')}`;
}
...
function localizedProviderMessage(provider, five, week) {
  ...
  const mapped = {
    auth_missing: 'authMissing', auth_unreadable: 'authMissing', auth_invalid: 'authMissing',
    reauth_required: 'reauth', unsupported_auth: 'unsupportedAuth', network_error: 'networkError',
    service_error: 'serviceError', invalid_response: 'invalidResponse', stale: 'staleNotice', desktop_required: 'nativeOnly'
  }[provider.state];
...
function renderSnapshot() {
  ...
  $('#source-account').textContent = snapshot.account?.displayName || t('unknownAccount');
  $('#source-plan').textContent = snapshot.account?.plan || '—';
  $('#source-detail').textContent = provider.authPathLabel || '~/.codex/auth.json';
...
async function refresh({ manual = false } = {}) {
  ...
  try {
    ...
    else if (nativeInvoke) state.snapshot = await invoke('refresh_monitor_data');
    else state.snapshot = {
      account: { displayName: t('unknownAccount'), plan: '—' },
      provider: { connected: false, state: 'desktop_required', message: t('nativeOnly'), authPathLabel: '~/.codex/auth.json' },
